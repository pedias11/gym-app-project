...
export class UserService {
  ...
  selectUserSubscription(email: string, subscription: string) {
    const headers = new HttpHeaders({
      'Content-Type': 'application/json'
    });
    const body = JSON.stringify({email, subscription });
    return this.http.post(`${API_URL}/select_subscription`, body, { headers });
  }
}
